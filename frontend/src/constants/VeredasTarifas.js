export const VEREDAS_ZIPAQUIRA = [
  "Barandillas",
  "Barro Blanco",
  "El Empalizado",
  "El Tunal",
...
export const KM_MINIMOS = 7;
export const VALOR_MINIMO = 80000;
export const VALOR_POR_KM_ADICIONAL = 11200;

export const DISTANCIAS_KM = {
...
export const normalizarVereda = (value) => {
...
export const calcularTarifaMinima = (origen, destino) => {
  const origenN = normalizarVereda(origen);
  const destinoN = normalizarVereda(destino);
  const distancia = DISTANCIAS_KM?.[origenN]?.[destinoN];
  if (distancia === undefined) {
    return {
      origen: origenN,
      destino: destinoN,
      distancia_km: 0,
      tarifa_minima: VALOR_MINIMO,
      km_minimos: KM_MINIMOS,
      valor_minimo: VALOR_MINIMO,
      valor_por_km_adicional: VALOR_POR_KM_ADICIONAL,
    };
  }
  const tarifa = Number(distancia) <= KM_MINIMOS
    ? VALOR_MINIMO
    : VALOR_MINIMO + ((Number(distancia) - KM_MINIMOS) * VALOR_POR_KM_ADICIONAL);
  return {
    origen: origenN,
    destino: destinoN,
    distancia_km: Number(Number(distancia).toFixed(1)),
    tarifa_minima: Math.round(tarifa),
    km_minimos: KM_MINIMOS,
    valor_minimo: VALOR_MINIMO,
    valor_por_km_adicional: VALOR_POR_KM_ADICIONAL,
  };
