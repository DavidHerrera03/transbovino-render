...
import { actualizarFinca, obtenerFincaPorId } from "../../../services/fincaService";
import { VEREDAS_ZIPAQUIRA } from "../../../constants/VeredasTarifas";

function EditarFinca({ usuario, salir, setVistaInterna, vistaInterna, fincaSeleccionada, setFincaSeleccionada }) {
  ...
  useEffect(() => {
    const cargar = async () => {
      if (!fincaSeleccionada?.id_finca) {
        setVistaInterna("misFincas");
        return;
      }
      try {
        setLoading(true);
        const finca = await obtenerFincaPorId(fincaSeleccionada.id_finca);
        setForm({
        ...
        alert(error.message || "No se pudo cargar la finca");
        setVistaInterna("misFincas");
      } finally {
        setLoading(false);
      }
    ...
    cargar();
  }, [fincaSeleccionada, setVistaInterna]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setGuardando(true);
      await actualizarFinca(fincaSeleccionada.id_finca, {
        ...form,
        municipio: "Zipaquira",
      ...
      alert(error.message || "No se pudo actualizar la finca");
    } finally {
      setGuardando(false);
    }
  };
    ...
    <CampesinoLayout usuario={usuario} salir={salir} setVistaInterna={setVistaInterna} vistaInterna={vistaInterna}>
      <div style={styles.page}>
        <div style={styles.header}>
          <h1 style={styles.title}>Actualizar finca</h1>
          <button style={styles.secondaryButton} onClick={() => { setFincaSeleccionada?.(null); setVistaInterna("misFincas"); }}>Volver</button>
        </div>

        <div style={styles.card}>
          {loading ? (
            <div>Cargando finca...</div>
          ) : (
            <form onSubmit={handleSubmit} style={styles.form}>
              ...
              <button type="submit" style={styles.primaryButton} disabled={guardando}>{guardando ? "Guardando..." : "Guardar cambios"}</button>
            </form>
          )}
        </div>
      </div>
